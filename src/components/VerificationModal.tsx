...
const VerificationModal = ({ open, onClose }: VerificationModalProps) => {
  ...
  const personalAnnual = 86.40;
  const companyMonthly = 41.67;
  const companyAnnual = 450.00;
      ...
      <div className="bg-card border border-border rounded-lg w-full max-w-lg p-6 space-y-5 relative max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="personal" onValueChange={v => setBadgeType(v as any)}>
          <TabsList className="w-full">
            <TabsTrigger value="personal" className="flex-1 flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> Pessoal</TabsTrigger>
            <TabsTrigger value="company" className="flex-1 flex items-center gap-1.5"><Building2 className="w-3.5 h-3.5" /> Empresa</TabsTrigger>
          </TabsList>
          <TabsContent value="personal" className="space-y-3 pt-3">
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setPlanType("monthly")} className={`p-4 rounded-xl border-2 text-center transition-all ${planType === "monthly" ? "border-[#1D9BF0] bg-[#1D9BF0]/10" : "border-border"}`}>
              ...
              <button onClick={() => setPlanType("annual")} className={`p-4 rounded-xl border-2 text-center transition-all relative ${planType === "annual" ? "border-[#1D9BF0] bg-[#1D9BF0]/10" : "border-border"}`}>
          ...
          <TabsContent value="company" className="space-y-3 pt-3">
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setPlanType("monthly")} className={`p-4 rounded-xl border-2 text-center transition-all ${planType === "monthly" ? "border-[#FFD700] bg-[#FFD700]/10" : "border-border"}`}>
              ...
              <button onClick={() => setPlanType("annual")} className={`p-4 rounded-xl border-2 text-center transition-all relative ${planType === "annual" ? "border-[#FFD700] bg-[#FFD700]/10" : "border-border"}`}>
