...
const AdminTradingFees = lazy(() => import("@/components/admin/AdminTradingFees"));
const AdminIndexDerivatives = lazy(() => import("@/components/admin/AdminIndexDerivatives"));
const AdminBranding = lazy(() => import("@/components/admin/AdminBranding"));
const AdminRevenueSplits = lazy(() => import("@/components/admin/AdminRevenueSplits"));
const AdminMiniSites = lazy(() => import("@/components/admin/AdminMiniSites"));
const AdminApiKeys = lazy(() => import("@/components/admin/AdminApiKeys"));
const AdminCopyright = lazy(() => import("@/components/admin/AdminCopyright"));
const AdminPremiumSlugs = lazy(() => import("@/components/admin/AdminPremiumSlugs"));
...
import {
  ...
  Zap, Trophy, Wallet, Flame, Brain, Power, Megaphone as MegaphoneIcon, Image, Key, Crown
} from "lucide-react";
...
const Admin = () => {
  ...
  // Jackpot winner percentages (up to 10 winners)
  ...
  const handleEditVideoPrice = (video: any) => {
    ...
    setEditPaywall(video.paywall_price);
    setEditSharePrice(video.share_price);
  };

  const handleSaveVideoPrice = async () => {
    if (!editingVideoId) return;
    const { error } = await supabase.from("videos").update({
      paywall_price: editPaywall,
      share_price: editSharePrice,
    } as any).eq("id", editingVideoId);
    if (error) toast.error(error.message);
    else toast.success("Prices updated!");
    setEditingVideoId(null);
  };

  const handleSetFree = async (videoId: string) => {
    const { error } = await supabase.from("videos").update({ paywall_price: 0, share_price: 0 } as any).eq("id", videoId);
    if (error) toast.error(error.message);
    else toast.success("Video set to FREE!");
  };

  // ─── Jackpot Trigger ───
  const handleTriggerJackpot = async () => {
    if (!jackpotPool || jackpotPool.total_amount <= 0) {
      toast.error("Jackpot pool is empty!");
      return;
    }
    const totalPct = winnerPcts.slice(0, winnerCount).reduce((s, p) => s + p, 0);
    if (Math.abs(totalPct - 100) > 0.01) {
      toast.error(`Winner percentages must total 100%. Current: ${totalPct}%`);
      return;
    }
    ...
    try {
      ...
      if (!tickets || tickets.length === 0) {
        toast.error("No active tickets to draw!");
        return;
      }
      ...
      for (let i = 0; i < winners.length; i++) {
        ...
        const pct = winnerPcts[i] / 100;
        const amount = winAmount * pct;

        const { data: wallet } = await supabase.from("wallets").select("balance").eq("user_id", winner.user_id).maybeSingle();
        if (wallet) {
          await supabase.from("wallets").update({ balance: wallet.balance + amount }).eq("user_id", winner.user_id);
        } else {
          await supabase.from("wallets").insert({ user_id: winner.user_id, balance: amount });
        }
      ...
      await supabase.from("jackpot_pool" as any).update({
        total_amount: 0,
        last_winner_id: (winners[0] as any).user_id,
        last_winner_amount: winAmount,
        last_drawn_at: new Date().toISOString(),
      } as any).eq("id", 1);

      toast.success(`🎉 Jackpot drawn! $${winAmount.toFixed(2)} split among ${winners.length} winner(s)`);
    } catch (err: any) {
      toast.error(err.message || "Jackpot draw failed");
    } finally {
      setTriggeringJackpot(false);
    }
  };
  ...
  const loadBoostRace = async () => {
    ...
    const { data } = await supabase
      .from("videos")
      .select("id, title, ticker, boost_count, thumbnail_url")
      .gt("boost_count", 0)
      .order("boost_count", { ascending: false })
      .limit(20);
    setBoostRace(data || []);
    setBoostRaceLoading(false);
  };

  // ─── Master Wallet Loader ───
  ...
  const handleBroadcast = () => {
    ...
    // Store broadcast in localStorage for now (could be expanded to DB)
    ...
    toast.success("Broadcast sent! (stored locally)");
    setBroadcastMsg("");
  };
        ...
        <Tabs defaultValue="analytics" className="space-y-4">
          <TabsList className="bg-secondary flex-wrap">
            ...
            <TabsTrigger value="visual">
              <Palette className="w-3.5 h-3.5 mr-1" />Visual
            </TabsTrigger>
            <TabsTrigger value="content">Content</TabsTrigger>
            <TabsTrigger value="reports" className="relative">
              Reports
              {pendingReports.length > 0 && (
                <span className="ml-1 bg-destructive text-destructive-foreground text-[8px] font-bold px-1.5 py-0.5 rounded-full">
            ...
            <TabsTrigger value="broadcast">
              <Megaphone className="w-3.5 h-3.5 mr-1" />Broadcast
            </TabsTrigger>
            <TabsTrigger value="admins" onClick={loadAdmins}>Admins</TabsTrigger>
            <TabsTrigger value="jackpot" onClick={loadMasterWallet}>
              <Zap className="w-3.5 h-3.5 mr-1" />Jackpot
            </TabsTrigger>
            <TabsTrigger value="boostrace" onClick={loadBoostRace}>
              <Flame className="w-3.5 h-3.5 mr-1" />Boost Race
            </TabsTrigger>
            <TabsTrigger value="masterwallet" onClick={loadMasterWallet}>
              <Wallet className="w-3.5 h-3.5 mr-1" />Master Wallet
            </TabsTrigger>
            <TabsTrigger value="aibrain" onClick={loadAiSettings}>
              <Brain className="w-3.5 h-3.5 mr-1" />AI Brain
            </TabsTrigger>
            <TabsTrigger value="adnetwork">
              <Image className="w-3.5 h-3.5 mr-1" />Ad Network
            </TabsTrigger>
            <TabsTrigger value="tradingfees">
              <DollarSign className="w-3.5 h-3.5 mr-1" />Trading Fees
            </TabsTrigger>
            <TabsTrigger value="indexderivatives">
              <TrendingUp className="w-3.5 h-3.5 mr-1" />Index & Derivatives
            </TabsTrigger>
            <TabsTrigger value="branding">
              <Palette className="w-3.5 h-3.5 mr-1" />Branding
            </TabsTrigger>
            <TabsTrigger value="splits">
              <Percent className="w-3.5 h-3.5 mr-1" />Revenue Splits
            </TabsTrigger>
            <TabsTrigger value="minisites">
              <Globe className="w-3.5 h-3.5 mr-1" />Mini-Sites
            </TabsTrigger>
            <TabsTrigger value="apikeys">
              <Key className="w-3.5 h-3.5 mr-1" />API Keys
            </TabsTrigger>
            <TabsTrigger value="premiumslugs">
              <Crown className="w-3.5 h-3.5 mr-1" />Nomes Premium
            </TabsTrigger>
            <TabsTrigger value="copyright">
              <ShieldAlert className="w-3.5 h-3.5 mr-1" />Copyright
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="analytics">
            {!analytics ? (
              <div className="text-center py-10 text-muted-foreground text-sm">Click Analytics tab to load data...</div>
            ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-foreground uppercase">
                        {(settings as any).trading_paused ? "⚠️ TRADING PAUSED — SANCTIONS MODE" : "✅ TRADING ACTIVE"}
                      </h3>
                      <p className="text-[10px] text-muted-foreground">
                        Kill-switch for all buy/sell operations. Use in case of sanctions or regulatory compliance.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => updateSettings.mutate({ trading_paused: !(settings as any).trading_paused } as any)}
                    className={`px-6 py-3 rounded-full font-black text-sm transition-colors ${
                      (settings as any).trading_paused
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide">
                  <button onClick={() => setCategoryFilter(null)}
                    className={`text-[10px] font-bold px-2 py-1 rounded transition-colors ${!categoryFilter ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-muted"}`}>
                    All
                  </button>
                  {categories.map((cat) => (
                    <button key={cat.id} onClick={() => setCategoryFilter(cat.id)}
                      className={`text-[10px] font-bold px-2 py-1 rounded whitespace-nowrap transition-colors ${categoryFilter === cat.id ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-muted"}`}>
                      {cat.name}
                    </button>
                  ))}
                </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border text-left">
                      ...
                      <th className="py-2 px-2 text-muted-foreground font-semibold text-center">Exchange</th>
                      <th className="py-2 px-2 text-muted-foreground font-semibold text-center">DEX</th>
                      ...
                      <tr key={video.id} className={`border-b border-border/50 hover:bg-muted/50 ${(video as any).under_review ? "bg-destructive/5" : ""}`}>
                        ...
                        <td className="py-1.5 px-2 font-mono">
                          {editingVideoId === video.id ? (
                            <input type="number" value={editPaywall} onChange={e => setEditPaywall(parseFloat(e.target.value) || 0)}
                              className="w-16 bg-secondary border border-border rounded px-1 py-0.5 text-xs" />
                          ) : (
                            <span className={video.paywall_price === 0 ? "text-primary font-bold" : ""}>
                              {video.paywall_price === 0 ? "FREE" : `$${video.paywall_price.toFixed(2)}`}
                            </span>
                          )}
                        </td>
                        <td className="py-1.5 px-2 font-mono">
                          {editingVideoId === video.id ? (
                            <input type="number" value={editSharePrice} onChange={e => setEditSharePrice(parseFloat(e.target.value) || 0)}
                              className="w-16 bg-secondary border border-border rounded px-1 py-0.5 text-xs" />
                          ) : (
                        ...
                        <td className="py-1.5 px-2 text-center">
                          <button onClick={() => updateVideo.mutate({ id: video.id, featured: !video.featured })}
                            className={`p-1 rounded transition-colors ${video.featured ? "text-primary" : "text-muted-foreground hover:text-foreground"}`}>
                            <Star className={`w-4 h-4 ${video.featured ? "fill-primary" : ""}`} />
                          </button>
                        </td>
                        <td className="py-1.5 px-2 text-center">
                          <button onClick={() => updateVideo.mutate({ id: video.id, blocked: !video.blocked })}
                            className={`p-1 rounded transition-colors ${video.blocked ? "text-destructive" : "text-muted-foreground hover:text-foreground"}`}>
                            <Ban className="w-4 h-4" />
                          </button>
                        </td>
                        <td className="py-1.5 px-2 text-center">
                          <button onClick={() => updateVideo.mutate({ id: video.id, exchange_active: !video.exchange_active })}
                            className={`px-2 py-0.5 text-[10px] font-bold rounded transition-colors ${video.exchange_active ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                            {video.exchange_active ? "ON" : "OFF"}
                          </button>
                        </td>
                        <td className="py-1.5 px-2 text-center">
                          <button onClick={() => updateVideo.mutate({ id: video.id, dex_mode: !(video as any).dex_mode } as any)}
                            className={`px-2 py-0.5 text-[10px] font-bold rounded transition-colors ${(video as any).dex_mode ? "bg-accent text-accent-foreground" : "bg-muted text-muted-foreground"}`}>
                            {(video as any).dex_mode ? "DEX" : "CEX"}
                          </button>
                        </td>
                        <td className="py-1.5 px-2 text-center">
                          <div className="flex items-center justify-center gap-1">
                            {editingVideoId === video.id ? (
                              ...
                              <>
                                <button onClick={() => handleEditVideoPrice(video)} className="text-primary hover:text-primary/80" title="Edit prices">
                                  <Edit3 className="w-3.5 h-3.5" />
                                </button>
                                <button onClick={() => handleSetFree(video.id)} className="text-accent-foreground hover:opacity-80" title="Set FREE">
              ...
              {(reports || []).length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-border text-left">
                        <th className="py-2 px-2 text-muted-foreground font-semibold">Video ID</th>
                        <th className="py-2 px-2 text-muted-foreground font-semibold">Reason</th>
                        <th className="py-2 px-2 text-muted-foreground font-semibold">Date</th>
                        <th className="py-2 px-2 text-muted-foreground font-semibold text-center">Status</th>
                        <th className="py-2 px-2 text-muted-foreground font-semibold text-center">Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(reports || []).map((r: any) => (
                        <tr key={r.id} className={`border-b border-border/50 ${!r.reviewed ? "bg-destructive/5" : ""}`}>
                          <td className="py-1.5 px-2 font-mono text-[10px] max-w-[120px] truncate">
                            <Link to={`/video/${r.video_id}`} className="text-primary hover:underline">{r.video_id?.slice(0, 8)}...</Link>
                          </td>
                          <td className="py-1.5 px-2 text-foreground">{r.reason}</td>
                          <td className="py-1.5 px-2 text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</td>
                          <td className="py-1.5 px-2 text-center">
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${r.reviewed ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}>
                              {r.reviewed ? "RESOLVED" : "PENDING"}
                            </span>
                          </td>
                          <td className="py-1.5 px-2 text-center">
                            {!r.reviewed && (
                              <button onClick={() => resolveReport.mutate({ id: r.id, reviewed: true })}
          ...
          <TabsContent value="flagged">
            <div className="border border-border rounded-lg p-4 bg-card space-y-3">
              <h2 className="text-sm font-bold text-card-foreground uppercase flex items-center gap-2">
                ...
                Videos Under Review ({(flaggedVideos || []).length})
              </h2>
              {(flaggedVideos || []).length === 0 ? (
                <p className="text-xs text-muted-foreground py-4 text-center">No flagged videos.</p>
              ) : (
                <div className="space-y-3">
                  {(flaggedVideos || []).map((v: any) => (
                    <div key={v.id} className="flex items-center justify-between bg-destructive/5 rounded-lg p-3 border border-destructive/20">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-destructive font-mono">{v.report_count} reports</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Link to={`/video/${v.id}`} className="text-[10px] text-primary hover:underline">View</Link>
                        <button onClick={() => updateVideo.mutate({ id: v.id, blocked: true })}
                          className="text-[10px] bg-destructive text-destructive-foreground px-2 py-1 rounded font-bold">Block</button>
                        <button onClick={() => handleClearReview(v.id)}
                          className="text-[10px] bg-primary text-primary-foreground px-2 py-1 rounded font-bold">Clear</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <TabsContent value="broadcast">
            <div className="border border-border rounded-lg p-4 bg-card space-y-4">
              ...
              <p className="text-[10px] text-muted-foreground">Send a message visible to all users on the platform.</p>
              <textarea
                value={broadcastMsg}
                onChange={e => setBroadcastMsg(e.target.value)}
                placeholder="Type your broadcast message..."
                className="w-full bg-secondary text-foreground text-sm border border-border rounded px-3 py-2 min-h-[80px] focus:outline-none focus:ring-1 focus:ring-primary"
              />
              <button onClick={handleBroadcast} disabled={!broadcastMsg.trim()}
                className="bg-primary text-primary-foreground px-4 py-2 rounded text-xs font-bold hover:bg-primary/90 disabled:opacity-50">
                Send Broadcast
              </button>

              <div className="border-t border-border pt-3">
                <h3 className="text-xs font-bold text-muted-foreground uppercase mb-2">Recent Broadcasts</h3>
                {JSON.parse(localStorage.getItem("admin_broadcasts") || "[]").slice(0, 5).map((b: any, i: number) => (
                  <div key={i} className="bg-secondary/50 rounded p-2 mb-2">
                    <p className="text-xs text-foreground">{b.message}</p>
                    <p className="text-[10px] text-muted-foreground mt-1">{new Date(b.date).toLocaleString()} — {b.by}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="border border-border rounded-lg p-4 bg-card space-y-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Email of user to add as admin..."
                ...
                <button onClick={handleAddAdmin} disabled={addingAdmin || !newAdminEmail.trim()}
                  className="flex items-center gap-1.5 bg-primary text-primary-foreground px-4 py-2 rounded text-xs font-bold hover:bg-primary/90 disabled:opacity-50">
              ...
              {adminsLoading ? (
                <p className="text-xs text-muted-foreground py-4 text-center">Loading admins...</p>
              ) : admins.length === 0 ? (
                <p className="text-xs text-muted-foreground py-4 text-center">Click this tab to load admin list.</p>
              ) : (
                <div className="space-y-2">
                  {admins.map((a: any) => (
                    <div key={a.id} className="flex items-center justify-between bg-secondary/50 rounded-lg px-4 py-3">
                      <div>
                        <p className="text-xs font-bold text-foreground">{a.display_name}</p>
                        <p className="text-[10px] font-mono text-muted-foreground">{a.user_id}</p>
                      </div>
                      <button onClick={() => handleRemoveAdmin(a.id, a.user_id)}
            ...
            <div className="space-y-4">
              <div className="border border-border rounded-lg p-6 bg-card text-center space-y-4">
                <div className="flex items-center justify-center gap-2">
                  <Zap className="w-6 h-6 text-[hsl(var(--ticker-up))]" />
                  <h2 className="text-lg font-extrabold text-foreground uppercase">Jackpot Pool</h2>
                </div>
                <p className="text-4xl font-mono font-extrabold text-[hsl(var(--ticker-up))]">
                  ${(jackpotPool?.total_amount ?? 0).toFixed(2)} <span className="text-sm text-muted-foreground">USDC</span>
                </p>
                {jackpotPool?.last_drawn_at && (
                  <div className="text-xs text-muted-foreground space-y-1">
                    <p>Last draw: {new Date(jackpotPool.last_drawn_at).toLocaleString()}</p>
                    <p>Winner received: <span className="font-bold text-[hsl(var(--ticker-up))]">${(jackpotPool.last_winner_amount ?? 0).toFixed(2)}</span></p>
                  </div>
                )}

                {/* Winner configuration */}
                <div className="border-t border-border pt-4 text-left max-w-md mx-auto space-y-3">
                  <h3 className="text-xs font-bold text-foreground uppercase">Winners Configuration</h3>
                  <label className="block">
                    <span className="text-[10px] text-muted-foreground">Number of Winners (1–10)</span>
                    <input
                      type="number" min={1} max={10} value={winnerCount}
                      onChange={(e) => setWinnerCount(Math.max(1, Math.min(10, parseInt(e.target.value) || 1)))}
                      className="mt-1 w-full bg-secondary text-foreground text-sm font-mono border border-border rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary"
                    />
                  </label>
                  <div className="space-y-1.5">
                    {Array.from({ length: winnerCount }).map((_, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <span className="text-xs font-bold text-muted-foreground w-16">#{i + 1}</span>
                        <input
                          type="number" min={0} max={100} step={0.1}
                          value={winnerPcts[i]}
                          onChange={(e) => {
                            const next = [...winnerPcts];
                            next[i] = parseFloat(e.target.value) || 0;
                            setWinnerPcts(next);
                          }}
                          className="flex-1 bg-secondary text-foreground text-sm font-mono border border-border rounded px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary"
                        />
                        <span className="text-xs text-muted-foreground">%</span>
                      </div>
                    ))}
                    <p className={`text-[10px] font-mono ${Math.abs(winnerPcts.slice(0, winnerCount).reduce((s, p) => s + p, 0) - 100) < 0.01 ? "text-[hsl(var(--ticker-up))]" : "text-destructive"}`}>
                      Total: {winnerPcts.slice(0, winnerCount).reduce((s, p) => s + p, 0).toFixed(1)}%
                    </p>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={triggeringJackpot || (jackpotPool?.total_amount ?? 0) <= 0}
                  className="px-8 py-3 bg-[hsl(var(--ticker-up))] text-white font-bold text-sm rounded-lg hover:opacity-90 transition-opacity disabled:opacity-40"
                >
                  {triggeringJackpot ? "Drawing..." : "🎰 TRIGGER JACKPOT DRAW"}
                </button>
              </div>
            </div>
          </TabsContent>
          ...
          <TabsContent value="boostrace">
            <div className="border border-border rounded-lg p-4 bg-card space-y-3">
              <h2 className="text-sm font-bold text-card-foreground uppercase flex items-center gap-2">
                ...
                Boost Race — Top 20 Videos
              </h2>
              {boostRaceLoading ? (
                <p className="text-xs text-muted-foreground py-4 text-center">Loading...</p>
              ) : boostRace.length === 0 ? (
                <p className="text-xs text-muted-foreground py-4 text-center">No boosts yet. Click tab to refresh.</p>
              ) : (
                <div className="space-y-2">
                  {boostRace.map((v: any, i: number) => (
                    <div key={v.id} className="flex items-center gap-3 bg-secondary/50 rounded-lg px-4 py-3">
                      <span className={`text-lg font-extrabold font-mono w-8 text-center ${i < 3 ? "text-[hsl(var(--gold))]" : "text-muted-foreground"}`}>
                        {i === 0 ? "🥇" : i === 1 ? "🥈" : i === 2 ? "🥉" : `#${i + 1}`}
                      </span>
                      <div className="w-12 h-7 rounded bg-muted overflow-hidden shrink-0">
                        {v.thumbnail_url && <img src={v.thumbnail_url} alt="" className="w-full h-full object-cover" />}
                      </div>
                      ...
                      <div className="text-right">
                        <p className="text-sm font-extrabold font-mono text-[hsl(var(--ticker-up))]">{v.boost_count}</p>
          ...
          <TabsContent value="masterwallet">
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="border border-border rounded-lg p-4 bg-card space-y-1 text-center">
                  <Wallet className="w-5 h-5 text-primary mx-auto" />
                  <p className="text-2xl font-mono font-extrabold text-[hsl(var(--gold))]">
                    ${(masterWallet?.totalPlatform ?? 0).toFixed(2)}
                  </p>
                  <p className="text-[10px] text-muted-foreground uppercase">Platform Revenue (50% Boosts)</p>
                </div>
                <div className="border border-border rounded-lg p-4 bg-card space-y-1 text-center">
                  <Zap className="w-5 h-5 text-[hsl(var(--ticker-up))] mx-auto" />
                  <p className="text-2xl font-mono font-extrabold text-foreground">{masterWallet?.totalBoosts ?? 0}</p>
                  <p className="text-[10px] text-muted-foreground uppercase">Total Boosts</p>
                </div>
                <div className="border border-border rounded-lg p-4 bg-card space-y-1 text-center">
                  <Trophy className="w-5 h-5 text-[hsl(var(--gold))] mx-auto" />
                  <p className="text-2xl font-mono font-extrabold text-foreground">{masterWallet?.totalTickets ?? 0}</p>
                  <p className="text-[10px] text-muted-foreground uppercase">Active Tickets</p>
                </div>
              </div>
              ...
              <div className="border border-border rounded-lg p-4 bg-card space-y-3">
                ...
                <div className="space-y-2 text-xs font-mono">
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Boost Revenue (50% per boost)</span>
                    <span className="font-bold">${(masterWallet?.totalPlatform ?? 0).toFixed(2)} USDC</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Jackpot Pool (35% per boost)</span>
                    <span className="font-bold text-[hsl(var(--ticker-up))]">${(jackpotPool?.total_amount ?? 0).toFixed(2)} USDC</span>
                  </div>
                  <div className="flex justify-between py-2">
                    <span className="text-muted-foreground">Creator Payouts (15% per boost)</span>
                    <span className="font-bold">${((masterWallet?.totalBoosts ?? 0) * 0.075).toFixed(2)} USDC</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* ─── AI BRAIN CONTROL TAB ─── */}
          <TabsContent value="aibrain">
            <div className="space-y-4">
              <div className="border border-border rounded-lg p-4 bg-card space-y-4">
                <h2 className="text-sm font-bold text-card-foreground uppercase flex items-center gap-2">
                  <Brain className="w-4 h-4 text-primary" />
                  AI Models — Content Moderation
                </h2>
                {aiLoading || !aiSettings ? (
                  <p className="text-xs text-muted-foreground py-4 text-center">Click tab to load AI settings...</p>
                ) : (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {[
                        { key: "gemini_enabled", label: "Google Gemini", desc: "Default model — fast, multimodal", color: "text-blue-500" },
                        { key: "chatgpt_enabled", label: "OpenAI ChatGPT", desc: "Advanced reasoning & analysis", color: "text-green-500" },
                        { key: "claude_enabled", label: "Anthropic Claude", desc: "Safety-focused, nuanced content", color: "text-orange-500" },
                        { key: "deepseek_enabled", label: "DeepSeek", desc: "Cost-effective alternative", color: "text-purple-500" },
                      ].map((model) => (
                        <div key={model.key} className="flex items-center justify-between bg-secondary/50 rounded-lg px-4 py-3">
                          <div className="flex items-center gap-3">
                            <Power className={`w-4 h-4 ${aiSettings[model.key] ? "text-[hsl(var(--ticker-up))]" : "text-muted-foreground"}`} />
                            <div>
                          ...
                          <button
                            onClick={() => updateAiSetting(model.key, !aiSettings[model.key])}
                            className={`px-3 py-1 text-[10px] font-bold rounded-full transition-colors ${
                              aiSettings[model.key]
                                ? "bg-[hsl(var(--ticker-up))] text-white"
                                : "bg-muted text-muted-foreground"
                            }`}
                          >
                            {aiSettings[model.key] ? "ON" : "OFF"}
                          </button>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="border-t border-border pt-4 space-y-3">
                      <h3 className="text-xs font-bold text-muted-foreground uppercase">Features</h3>
                      <div className="flex items-center justify-between bg-secondary/50 rounded-lg px-4 py-3">
                        <div>
                          <p className="text-xs font-bold text-foreground">Auto Content Moderation</p>
                          <p className="text-[10px] text-muted-foreground">Automatically flag inappropriate content</p>
                        </div>
                        <button
                          onClick={() => updateAiSetting("moderation_active", !aiSettings.moderation_active)}
                          className={`px-3 py-1 text-[10px] font-bold rounded-full transition-colors ${
                            aiSettings.moderation_active ? "bg-[hsl(var(--ticker-up))] text-white" : "bg-muted text-muted-foreground"
                          }`}
                        >
                          {aiSettings.moderation_active ? "ON" : "OFF"}
                        </button>
                      </div>
                      <div className="flex items-center justify-between bg-secondary/50 rounded-lg px-4 py-3">
                        <div>
                          <p className="text-xs font-bold text-foreground">Sentiment Analysis</p>
                          <p className="text-[10px] text-muted-foreground">Analyze viewer sentiment in comments</p>
                        </div>
                        <button
                          onClick={() => updateAiSetting("sentiment_analysis", !aiSettings.sentiment_analysis)}
                          className={`px-3 py-1 text-[10px] font-bold rounded-full transition-colors ${
                            aiSettings.sentiment_analysis ? "bg-[hsl(var(--ticker-up))] text-white" : "bg-muted text-muted-foreground"
                          }`}
                        >
                          {aiSettings.sentiment_analysis ? "ON" : "OFF"}
                        </button>
                      </div>
                      <div className="bg-secondary/50 rounded-lg px-4 py-3">
                        <label className="text-xs font-bold text-foreground block mb-1">Auto-Flag Threshold</label>
                        <p className="text-[10px] text-muted-foreground mb-2">Content scoring above this value gets flagged (0.0–1.0)</p>
                        <input
                          type="number"
                          step="0.05"
                          min="0"
                          max="1"
                          value={aiSettings.auto_flag_threshold}
                          onChange={(e) => updateAiSetting("auto_flag_threshold", parseFloat(e.target.value) || 0.8)}
                          className="w-24 bg-secondary border border-border rounded px-2 py-1 text-xs font-mono text-foreground"
                        />
                      </div>
                    </div>
                  </div>
          ...
          <TabsContent value="adnetwork">
            <div className="border border-border rounded-lg p-4 bg-card space-y-4">
              <h2 className="text-sm font-bold text-card-foreground uppercase flex items-center gap-2">
                ...
                Ad Campaigns ({(allAds || []).length})
              </h2>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {["billboard", "sidebar", "overlay", "sponsored"].map((slot) => {
                  const count = (allAds || []).filter((a: any) => a.slot_type === slot && a.active).length;
                  return (
                    <div key={slot} className="border border-border rounded-lg p-3 bg-secondary/50 text-center">
                      <p className="text-lg font-mono font-bold text-foreground">{count}</p>
                      <p className="text-[10px] text-muted-foreground uppercase">{slot} Active</p>
                    </div>
                  );
                })}
              </div>
              {(allAds || []).length === 0 ? (
                <p className="text-xs text-muted-foreground py-4 text-center">No ad campaigns yet. Advertisers can create campaigns from their dashboard.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-border text-left">
                        ...
                        <th className="py-2 px-2 text-muted-foreground font-semibold">CTR</th>
                        <th className="py-2 px-2 text-muted-foreground font-semibold text-center">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(allAds || []).map((ad: any) => (
                        <tr key={ad.id} className="border-b border-border/50 hover:bg-muted/50">
                          ...
                          <td className="py-1.5 px-2 font-mono">{ad.impressions.toLocaleString()}</td>
                          <td className="py-1.5 px-2 font-mono">{ad.clicks}</td>
                          <td className="py-1.5 px-2 font-mono">
                            {ad.impressions > 0 ? ((ad.clicks / ad.impressions) * 100).toFixed(1) : "0.0"}%
                          </td>
                          <td className="py-1.5 px-2 text-center">
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${ad.active ? "bg-[hsl(var(--ticker-up))]/20 text-[hsl(var(--ticker-up))]" : "bg-muted text-muted-foreground"}`}>
          ...
          {/* ─── INDEX & DERIVATIVES TAB ─── */}
          <TabsContent value="indexderivatives">
            <AdminIndexDerivatives />
          </TabsContent>
          {/* ─── BRANDING TAB ─── */}
          ...
          <TabsContent value="copyright">
            <Suspense fallback={<div className="text-center py-10 text-muted-foreground text-sm">Loading...</div>}>
              <AdminCopyright />
            </Suspense>
          </TabsContent>
